...
export default function ProgramPage() {
  ...
  useEffect(() => { if (!isCoach) return; getDocs(query(collection(db, "players"), orderBy("name"))).then((snap) => { const loaded = snap.docs.map((item) => ({ id: item.id, ...(item.data() as Omit<PlayerDoc, "id">) })); setPlayers(loaded); }); }, [isCoach]);
  ...
  async function handleSave() {
    ...
    try {
      await addDoc(collection(db, "programs"), {
        ...
        assignedPlayerIds: players.map((player) => player.id),
        assignedAll: true,
